    ...
    if uploaded_file is not None:
        ...
        for row in range(num_rows):
            ...
            for col in range(num_columns):
                ...
                if idx < len(signal_labels):
                    ...
                    columns[col].line_chart(
                        np.array(signals[idx][:1000]), use_container_width=True
                    )
        ...
        if st.button("diagnose for schzophrenia"):
            ...
            if response.status_code == 200:
                ...
                st.error(
                    f"Failed to call the FastAPI Endpoint. Status code: {response.status_code}"
                )
            ...
            skizo_positive_text = (
                "## You have a high possibility of having schizophrenia"
            )
            skizo_negative_text = (
                "## You have a low possibility of having schizophrenia"
            )
