...
def plot_heatmap(denorm_image, pred, heatmap, channel_name):
    ...
    plt.savefig(
        f"grad_cams/{channel_name}.png", bbox_inches="tight", pad_inches=1, format="png"
    )
...
def mutli_grads(model, images, predictions, channel_names, size):
    for i in range(len(images)):
        ...
        denorm_image = images[i].permute(1, 2, 0) * np.array(
            (0.5, 0.5, 0.5)
...
checkpoint = torch.load("resnet-finetune-adam-epoch_20", map_location=device)
...
def plot_predictions(prediction_dict):
    ...
    colors = [
        "tomato" if value > 0.5 else "skyblue" for value in prediction_dict.values()
    ]
    values = [
        values if values > 0.5 else 1 - values for values in prediction_dict.values()
    ]
    bars = plt.bar(prediction_dict.keys(), values, color=colors)
    plt.yticks(color="white")
    plt.xticks(rotation=90, color="white")
    plt.bar_label(
        plt.bar(prediction_dict.keys(), values, color=colors),
        labels=[round(value, 2) for value in values],
        padding=3,
        color="white",
    )
    plt.ylim(0, 1.5)
